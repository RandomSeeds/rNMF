...
#' @param trim A number between 0 and 1. Percentage of contents of X to be trimmed. If trim = 0. 
#' @param ini.H A numeric matrix. Initial right matrix H in A ~= W \%*\% H. Must have the same number of columns as A and k rows.
#' @param ini.W A numeric matrix. Initial left matrix W in A ~= W \%*\% H. Must have the same number of rows as A, and k columns. Only one of ini.H and ini.W is used. 
#' @param ini.zeta A logical matrix. Must have the same dimension as A. "FALSE" entries in ini.zeta corresponds to an entry marked as outliers and will be trimmed in the first iteration. The number of "FALSE" in ini.zeta must be <=  round(trim * p * n).
#' @param my.seed Numeric. The random seed for initialization of W or H. Ignored if ini.H or ini.W are set.
#' @param variation Character string. Which variation to use? The options are: 'cell', 'col', 'smooth', 'row'.
#' @param quiet Logical. If 'TRUE' display a short report on screen after the function is run.
    ...
    variation = "cell", quiet = FALSE, nreg = 1, p1 = NA, n1 = NA)
{
    ...
    checkargs(A, k, alpha, beta, maxit, tol, trim, ini.W, ini.zeta, my.seed, variation, quiet, nreg, p, n)
    
    ## Create a data frame of 4 columns: value = entries of A; (x,y) = coordinates; outs = is it an outlier?
    ...
    return(invisible(list(W = W, H = H, fit = fit,
                          trimmed1 = to.trim, trimmed2 = to.trim, niter = i)))
}
