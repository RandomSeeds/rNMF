...
#' 
#' Performs robust non-negative matrix factorizaion on a non-negative
#' matrix A. A~=WH, where A is a p by n matrix, W is a p by k matrix
#' and H is a k by n matrix.
#' (This parts to be finished)
#'
#' @param A The input non-negative matrix to be factorized. \code{A}
#' @param k The number of columns of W \code{k}
#' @param alpha Non-negative number. Controls ||W||_2^2 \code{alpha}
#' @param beta Non-negative number. Controls sparsity of H \code{beta}
#' @param maxit Positive integer. Maximum number of iterations.\code{maxit}
#' @param tol Tolerance for convergence \code{tol}
#' @param trim Logical. Trim or not?\code{trim}
#' @param ini.H Initial H \code{ini.H}
#' @param ini.W Initial W \code{ini.W}
#' @param ini.zeta Initial zeta \code{ini.zeta}
#' @param my.seed If missing initial W, use this seed to generate it \code{my.seed}
#' @param my.mode Which variation to use? one of the following strings 'cell','col', 'smooth', 'row', 'rowsmooth' \code{my.mode}
#' @param quiet Display a short report on screen after the function is run? \code{quiet}
#' @param nreg number of runs in "cell" variation. \code{nreg}
#' @param p1 Don't ask questions. \code{p1}
#' @param n1 Don't ask questions. \code{n1}
#' 
#' @return output W and H.
#'
#' @keywords keywords
#'
#' @export
...
rnmf = function(A, k = 5, alpha = 0, beta = 0, maxit = 50, tol = 0.01, trim = FALSE, ini.H = NULL, ini.W = NULL, ini.zeta = NULL, my.seed = NULL, my.mode = "cell", quiet = FALSE, nreg = 1, p1 = NA, n1 = NA){
    ## ------------------------------------------------
...
## -------------------
    ...
    if(my.mode == "cell" & trim > 0){
        ...
        for(i in 1:maxit){
            ...
            if(trim > 0){
                ...
                if(my.mode == "col" | my.mode == "all"){
                ...
                }else if(my.mode == "cellcol" | my.mode == "cellall"){
                    ...
                    C.s = rbind(t(A.s), matrix(0, nrow = k, ncol = p))
                    W = t(Nnls(M,C.s))
                }else if(my.mode == "row" | my.mode == "cellrow"){
                    ## No change
                }else{
            ...
            if(trim > 0){
                R = (A - W %*% H)^2
                if(my.mode == "col" | my.mode == "cellcol"){
                    ## No change
                }else if(my.mode == "cellrow" | my.mode == "cellall"){
                    ...
                    H.prev <- H
                    H = Nnls(M,C.s)
                }else if(my.mode == "row" | my.mode == "all"){
    ...
    if(quiet == TRUE){
        ...
        if(trim == FALSE){
            ...
            cat("Done. Time used:","\n")
            print(proc.time() - tic)
            cat("\n Trimming mode =\"", my.mode, "\". Proportion trimmed:",trim, "\n",
                "Input matrix dimension: ",p,"by",n, "\n",
                "Left matrix:",p,"by",k,". Right matrix: ",k,"by",n,"\n",
                "alpha = ",alpha,". beta = ",beta,"\n",
                "Number of max iterations = ",maxit,"\n",
...
## Example:
## res = RNMF(A, k, alpha = 0, beta = 0, maxit = 50, trim = 0.05, ini.H = NULL, my.seed = NULL, my.mode = "cellcol"){

## ------------------------------------------------
## Old versions. Not used any more. All of the following are included in the previous function RNMF. 
## ------------------------------------------------

## stNMF: The main sparse trimmed NMF algorithm.
## Model: A = WH + E.
## Improves the sparse alternating least square NMF algorithm (Kim & Park 2007).
## Objective function: min ||A-WH||_2^2 + \alpha * ||W||_2^2 + \beta * ||H(:,j)||_1^2, W,H >= 0
## Need to do next: 1. Initialization seeding selection. 2. Convergence criteria.
## arguments: k = ncol(W). maxit = number of iterations. trim = "trim the data?". trimp = "% to keep". my.seed = seed for initializeing H.
## stNMF = function(A, k, alpha = 0, beta = 0, maxit = 50, trim = FALSE, trimp = 0.9, ini.H = NULL, my.seed = NULL){
##     p = nrow(A)
##     n = ncol(A)
##     if(trim == TRUE){
##         trimed.row = matrix(NA, nrow = maxit, ncol = p - round(trimp * p))
##         trimed.col = matrix(NA, nrow = maxit, ncol = n - round(trimp * n))
##     }else{
##         trimed.row = NULL
##         trimed.col = NULL
##     }
...
##             R = (A - W %*% H)^2
##             rsqr = apply(R, 2, l2) ## Find SS of residuals of columns
##             to.trim = which(rank(rsqr) > round(trimp * n))
##             trimed.col[i,] = to.trim
##             A.trim = A[,-to.trim]
##             H.trim = H[,-to.trim]
##             ## Fit W with trimmed A and H. 
##             M.trim = rbind(t(H.trim), sqrt(alpha) * diag(k))
...
##             R = (A - W %*% H)^2
##             rsqr = apply(R, 1, l2) ## Find SS of residuals of rows
##             to.trim = which(rank(rsqr) > round(trimp * p))
##             trimed.row[i,] = to.trim
##             A.trim = A[-to.trim,]
##             W.trim = W[-to.trim,]
##             ## Fit H with trimmed A and W. 
##             M.trim = rbind(W.trim, sqrt(beta) * matrix(1, nrow = 1, ncol = k))
...
##     close(pb)
## #    J = nmlz(W) # Find the normalizing matrix of W
## #    return(list(W = W %*% J, H = solve(J) %*% H), )
##     return(list(W = W, H = H, trim.row = trimed.row, trim.col = trimed.col, SS = SS))
## }

## ## ---------------------------------
## ## second type of stNMF (trimming according to l2 norm of points residuals not l2 norm of columns of row residuals)
## ## ---------------------------------
## stNMF2 = function(A, k, alpha = 0, beta = 0, maxit = 10, trim = FALSE, trimp = 0.9, ini.H = NULL, my.seed = NULL){
##     p = nrow(A)
##     n = ncol(A)
##     if(trim == TRUE){
##         to.trim1 = vector("list", maxit)
...
##         if(trim == TRUE){
##             R = (A - W %*% H)^2
##             to.trim1[[i]] = which(rank(abs(R)) > round(trimp * n * p))
##             trim.col = unique((to.trim1[[i]] %/% p) + 1)
##             A.trim = A[,-trim.col]
##             H.trim = H[,-trim.col]
##             ## Fit W with trimmed A and H. 
...
##         if(trim == TRUE){
##             R = (A - W %*% H)^2
##             to.trim2[[i]] = which(rank(abs(R)) > round(trimp * n * p))
##             trim.row = unique(to.trim2[[i]] %% p)
##             A.trim = A[-trim.row,]
##             W.trim = W[-trim.row,]
##             ## Fit H with trimmed A and W. 
...
## #    J = nmlz(W) # Find the normalizing matrix of W
## #    return(list(W = W %*% J, H = solve(J) %*% H), )
##     return(list(W = W, H = H, trimmed1 = to.trim1, trimmed2 = to.trim2, SS = SS))
## }

## ## ---------------------------------
## ## Third type of stNMF (trimming according to l2 norm of points residuals not l2 norm of columns of row residuals)
## ## ---------------------------------
## stNMF3 = function(A, k, alpha = 0, beta = 0, maxit = 50, trim = FALSE, trimp = 0.95, ini.H = NULL, my.seed = NULL){
##     p = nrow(A)
##     n = ncol(A)
##     A.f = data.frame(pixel = as.vector(A), x = rep(1:p,n), y = rep(1:n, each = p), outs = FALSE)
##     if(trim == TRUE){
##         to.trim1 = vector("list", maxit)
##         to.trim2 = vector("list", maxit)
##     }else{
...
##             R = (A - W %*% H)^2
##             to.trim1[[i]] = which(rank(abs(R)) > round(trimp * n * p))
##             A.s = matrix(smoothing2(A.f, to.trim1[[i]], p, n, frame = TRUE)$pixel,p,n)
##             ## Fit W with trimmed A and H. 
##             C.s = rbind(t(A.s), matrix(0, nrow = k, ncol = p))
##             W = t(Nnls(M,C.s))
##         }
...
## #    J = nmlz(W) # Find the normalizing matrix of W
## #    return(list(W = W %*% J, H = solve(J) %*% H), )
##     return(list(W = W, H = H, trimmed1 = to.trim1,
##                 trimmed2 = to.trim2, SS = SS, fit = W %*% H))
## }
